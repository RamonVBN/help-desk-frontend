...
const technicianBaseProfileFormSchema = z.object({
  ...
  availableHours: z
    .array(z.string())
    .nonempty({ error: "Selecione os horários de atendimento." }),
  password: z.string().min(6).or(z.literal("")).optional(),
})

const createTechnicianProfileFormSchema =
  technicianBaseProfileFormSchema.extend({
    password: z
      .string()
...
type TechnicianUpdateProfileForm = z.infer<
  typeof technicianBaseProfileFormSchema
>
type TechnicianCreateProfileForm = z.infer<
  typeof createTechnicianProfileFormSchema
>
...
export function TechnicianProfilePage({ mode }: TechnicianProfileFormProps) {
  const schema =
    mode === "create"
      ? createTechnicianProfileFormSchema
      : technicianBaseProfileFormSchema
  ...
  const { mutate: updateTechProfile, isPending: isPendingTechProfile } =
    useMutation({
      mutationFn: ({
        ...
        email,
        availableHours,
      }: TechnicianUpdateProfileForm) =>
        api.put(`/users/${techId}`, {
          name,
          email,
          availableHours,
        }),
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col gap-4 lg:gap-6">
            <div className="flex flex-col lg:flex-row lg:items-center gap-3 justify-between ">
              <h1 className="font-bold text-xl leading-[140%] text-blue-800">
                Perfil de técnico
              </h1>
            ...
            <div className="flex flex-col gap-4 lg:gap-6 lg:flex-row lg:items-start">
                ...
                <div className="flex flex-col gap-4">
                  {techId && technician && (
                    <Avatar
                      ...
                      className="w-12 h-12 text-[1.3125rem]"
                    />
                  )}
                  ...
                  {!techId && (
                    <div>
                      ...
                      {errors.password ? (
                        ...
                        <span className="text-xs italic leading-[140%] text-gray-400">
                          Mínimo de 6 digitos
                        </span>
                      )}
                    </div>
              ...
              <Card.Root className="p-5 lg:p-6 flex gap-5 lg:flex-[30rem]">
                ...
                {errors.availableHours && (
                  <ErrorMessage>{errors.availableHours.message}</ErrorMessage>
                )}
              </Card.Root>
            </div>
          </div>
