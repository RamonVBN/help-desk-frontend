...
export function TechCalleds({ initialCalledsData }: TechCalledsProps) {
  ...
  const progressCalleds = techCalleds?.filter(
    (called) => called.status === "PROGRESS",
  )

  const openCalleds = techCalleds?.filter((called) => called.status === "OPEN")

  const closedCalleds = techCalleds?.filter(
    (called) => called.status === "CLOSED",
  )
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap">
            {progressCalleds?.map((called) => {
              return <TechCalledCard called={called} key={called.id} />
            })}
          </div>
        </div>
        <div className="flex flex-col gap-4">
